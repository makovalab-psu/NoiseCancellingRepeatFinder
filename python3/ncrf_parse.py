...
class Alignment(object):
		...
		for (ix,line) in enumerate(self.lines):
			if (line.startswith("# position ")):
				if (startIx == None):
					...
					raise ValueError("non-consecutive positional information for alignment at line %d:\n%s" \
	...
	def positional_stats(self):
		...
		for ix in range(startIx,endIx):
			...
			if (pos != expectedPos):
				raise ValueError("positional information out of order for alignment at line %d:\n%s" \
				               % (self.lineNumber,line))
	...
	def __lt__(self,other):
		selfEmpty  = not hasattr(self, "lineNumber")
		otherEmpty = not hasattr(other,"lineNumber")
		if (selfEmpty):  return True
		if (otherEmpty): return False
		if (self.lineNumber  != other.lineNumber):  return (self.lineNumber  < other.lineNumber)
		if (self.mRatio      != other.mRatio):      return (self.mRatio      < other.mRatio)
		if (self.nMatch      != other.nMatch):      return (self.nMatch      < other.nMatch)
		if (self.nMismatch   != other.nMismatch):   return (self.nMismatch   < other.nMismatch)
		if (self.nInsertions != other.nInsertions): return (self.nInsertions < other.nInsertions)
		if (self.nDeletions  != other.nDeletions):  return (self.nDeletions  < other.nDeletions)
		return True

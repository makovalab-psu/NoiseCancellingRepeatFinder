...
programName    = "ncrf_resolve_overlaps"
programVersion = "1.02.00"


def usage(s=None):
	message = """
usage: %s <alignment_summary..> [options]
...
def main():
	...
	for filename in inputFilenames:
		...
		for summary in read_summary(f,filename):
			...
			if (headLimit != None) and (summaryNum > headLimit):
				print("limit of %s summaries reached" % "{:,}".format(headLimit),file=stderr)
				break
		...
		for seq in seqOrder:
			for group in seqToGroups[seq]:
				...
				for summary in group:
					print(summary.line,file=stderr)
	...
	if (writeSingletonsSeparately):
		for motif in singletons:
			...
			motifF = open(motifFilename,"wt")
			print("writing to \"%s\"" % motifFilename,file=stderr)

			if (summaryHeaderLine != None):
				print(summaryHeaderLine,file=motifF)
			for group in subsetToGroups[subset]:
				for summary in group:
					print(summary.line,file=motifF)
	...
	if (outTemplate == None):
		...
		if (list(subsetToGroups) == []):
			print("no alignments to write to console",file=stderr)
	elif ("{motif}" not in outTemplate):
		outF = open(outTemplate,"wt")
		if (list(subsetToGroups) == []):
			print("no alignments to write to \"%s\"" % outTemplate,file=stderr)
		else:
			print("writing to \"%s\"" % outTemplate,file=stderr)
	else:
		outFilename = outTemplate.replace("{motif}","overlaps")
		outF = open(outFilename,"wt")
		if (list(subsetToGroups) == []):
			print("no alignments to write to \"%s\"" % outFilename,file=stderr)
		else:
			print("writing to \"%s\"" % outFilename,file=stderr)
	...
	for motifCount in motifCounts:
		...
		for subset in subsets:
			for group in subsetToGroups[subset]:
				if (isFirstGroup):
					if (summaryHeaderLine != None):
						print(summaryHeaderLine,file=outF)
					isFirstGroup = False
				else:
					print(file=outF)  # (line to separate groups)

				for summary in group:
					print(summary.line,file=outF)

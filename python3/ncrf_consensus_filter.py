...
from ncrf_parse import alignments,reverse_complement,canonical_motif, \
                       parse_probability,int_with_unit


programName    = "ncrf_consensus_filter"
programVersion = "1.02.00"


def usage(s=None):
	message = """
usage: ncrf_cat <output_from_NCRF> | %s [options]
	...
	for a in alignments(f,requireEof):
		...
		if (reportProgress != None):
			if (alignmentNum == 1) or (alignmentNum % reportProgress == 0):
				print("progress: testing alignment %s" \
				    % "{:,}".format(alignmentNum),file=stderr)
		...
		if ("consensus" in debug):
			print(file=stderr)
			print("%d score=%d" % (a.lineNumber,a.score),file=stderr)
...
def sliced_consensus_filter(f,motifsOfInterest,nameToMotif,sliceWidth,sliceStep):
	...
	if (reportMsa) and (not userHasBeenWarned):
		print("WARNING: sliced consensus doesn't report MSA, ignoring that request",file=stderr)
	...
	for a in alignments(f,requireEof):
		...
		for sliceNum in range(numSlices):
			...
			if ("consensus" in debug):
				print(file=stderr)
				print("%d score=%d slice.start=%d slice.end=%d" \
				    % (a.lineNumber,a.score,sliceStart,sliceEnd),file=stderr)
			...
			if (sliceConsensuses == []):
				...
				if ("consensus" in debug):
					for word in sliceConsensuses:
						print("consensus %s" % word,file=stderr)
...
def chunkify(motif,motifText,seqText):
	...
	if (motifPosInfo == None):
		...
		if ("motifpos" in debug):
			print("motifPos=%d%s (None)" % (motifPos,direction),file=stderr)
	else:
		...
		if ("motifpos" in debug):
			...
			print("motifPos=%d%s (%s)" \
			    % (motifPos,direction,alignedMotif),file=stderr)
	...
	if ("chunks" in debug):
		for (chunkIx,chunk) in enumerate(chunks):
			print("chunk[%d] = %s" % (chunkIx,str(chunk)),file=stderr)
	...
	for motifIx in range(motifLen):
		...
		if ("consensus" in debug):
			...
			for (count,seqNucs) in ixToTokens[motifIx]:
				s += ["%d:\"%s\"" % (count,seqNucs)]
			print("# [%d]%s tokensCount=%d %s" % \
			      (motifIx,
			       " !!!" if (bestCount < winnerThreshold*tokensCount) else "",
			       tokensCount," ".join(s)),file=stderr)
...
def abort_warn_about_named_motifs(a):
	print("ERROR: alignment at line %d contains non-ACGT:\n\"%s\"" % (a.lineNumber,a.motif),file=stderr)
	print("This is probably because the alignment file uses named motifs. If so, you"      ,file=stderr)
	print("need to specify the same <name>:<motif> options on this command line as were"   ,file=stderr)
	print("provided to the NCRF alignment command."                                        ,file=stderr)
	exit()
